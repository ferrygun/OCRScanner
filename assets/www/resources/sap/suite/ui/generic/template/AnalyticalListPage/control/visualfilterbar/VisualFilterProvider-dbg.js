...
sap.ui.define([
				...
				if (filterRestriction !== "interval" && isFilterable !== "false") {
					...
					for (var propertyFields in aFields[oProperty]) {
						...
						if (propertyFields.indexOf(V4Terms.ValueList) > -1 && aFields[oProperty][propertyFields].PresentationVariantQualifier) {
							...
							this._getAnnotationFromValueList(sEntityNameFull, isSelected, valueList, currentPropName, filterRestriction, isDropDown, isMandatory);
						}
					}
				}
			}
	...
	*/
	VisualFilterProvider.prototype._getAnnotationFromValueList = function (entityFullName, isSelected, valueListProperties, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, entityType) {
		...
		if (valueListProperties !== undefined) {
			...
			if (presentationVariantQualifierString) {
				...
				if (dimension) {
					...
					// Needs to be done
					annotationObject = this._createConsumeableObjectFromAnnotation(presentationVariantAnnotation, collectionPath, isSelected, parameters, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, entityTypeCollectionPath);
					if (annotationObject) {
	...
	*/
	...
	/*
		...
		and form the config.
	*/
	VisualFilterProvider.prototype._createConsumeableObjectFromAnnotation = function (presentationVariantAnnotation, collectionPath, isSelected, parameters, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, entityType) {
